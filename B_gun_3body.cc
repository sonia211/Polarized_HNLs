...
int main() {
	...
	int HNL_id = 15;	// choose 15 for tau, 25 for Higgs0
	int mu2_id=-13; 
	
	int tauMode = 3, tauPol = 1;
	...
	for (int a(0); a<mNvec.size(); ++a){ // loop on masses
		...
		for (int b(0); b<tau0vec.size(); ++b){ // loop on lifetimes
			...
			if(HNL_id == 15){
				...
				pythia.readString("15:name = HNL");
				pythia.readString("15:antiName = None");
				...
				pythia.readString("521:oneChannel = 1 1. 23 "+HNL_id_s2+" -13 111");
			...
			//if(!scanflag){
				...
				TFile *file;
				if (!scanflag) file = TFile::Open("./tree_Bgun_3body.root","recreate");
				//Event *event = &pythia.event;
				...
				//TBranch *b_meson_MT= Tmo->Branch("meson_MT",&meson_MT, "meson_MT/D");
			...
			for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
				...
				cout << "B statut : " << pythia.event[1].status() << endl;
				...
				meson_E = pythia.event[4].e();
				meson_M = pythia.event[4].m();
				//meson_MT = pythia.event[4].mT();
				//meson_P2 = pythia.event[4].p2(
				meson_PX = pythia.event[4].px();
				meson_PY = pythia.event[4].py();
				meson_PZ = pythia.event[4].pz();
				meson_PT = pythia.event[4].pT();
				meson_tau = pythia.event[4].tau();
			...
			//	std::cout << "HNL Status: " << pythia.event[2].status();
				...
				mu2_E = pythia.event[5].e();
				mu2_M = pythia.event[5].m();
				mu2_MT = pythia.event[5].mT();
				//mu2_P2 = pythia.event[d1].p2(
				mu2_PX = pythia.event[5].px();
				mu2_PY = pythia.event[5].py();
				mu2_PZ = pythia.event[5].pz();
				mu2_PT = pythia.event[5].pT();
				
				pi_E = pythia.event[6].e();
				pi_M = pythia.event[6].m();
				pi_MT = pythia.event[6].mT();
				//pi_P2 = pythia.event[d1].p2(
				pi_PX = pythia.event[6].px();
				pi_PY = pythia.event[6].py();
				pi_PZ = pythia.event[6].pz();
				pi_PT = pythia.event[6].pT();
